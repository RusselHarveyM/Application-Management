...
public class UserRepository : BaseRepository, IUserRepository
{
    ...
    public UserRepository(IUnitOfWork unitOfWork, BasecodeContext context, UserManager<IdentityUser> userManager,
    ...
    public async Task Create(User user)
    {
        ...
        var result = await _userManager.CreateAsync(identityUser, user.Password.Trim());

        if (result.Succeeded)
        {
            var checkIfRoleExists = await _roleManager.RoleExistsAsync(user.Role);
            ...
            else
            {
                ...
                if (createRole.Succeeded) await _userManager.AddToRoleAsync(identityUser, user.Role);
            }
    ...
    public async Task Delete(User user)
    {
        ...
        if (findUser != null) await _userManager.DeleteAsync(findUser);
    }
    ...
    {
        var userDB = GetDbSet<IdentityUser>().Where(x => x.UserName.ToLower().Equals(userName.ToLower())).AsNoTracking()
            .FirstOrDefault();
        var user = await _userManager.FindByNameAsync(userName);
        var isPasswordOK = await _userManager.CheckPasswordAsync(user, password);
        if (user == null || isPasswordOK == false) userDB = null;
        return userDB;
    }
