...
public class JobOpeningService : ErrorHandling, IJobOpeningService
{
    ...
    public JobOpeningService(IJobOpeningRepository repository, IMapper mapper,
        IQualificationService qualificationService, IResponsibilityService responsibilityService,
    ...
    public List<JobOpeningViewModel> GetJobsWithApplicationsSorted()
    {
        var data = _repository.GetJobsWithApplications()
            .Select(m => _mapper.Map<JobOpeningViewModel>(m))
            .ToList();
        ...
        data.Sort((job1, job2) => DateTime.Compare((DateTime)job2.UpdatedTime, (DateTime)job1.UpdatedTime));

        // Take the first 7 job openings from the sorted list
        var recentJobs = data.Take(7).ToList();
        return recentJobs;
    }
    ...
    public void UpdateJobOpeningUsers(int jobOpeningId, List<string> assignedUserIds)
    {
        ...
        var hasNewAssignedUsers = assignedUserIds.Except(currentAssignedUserIds).Any();

        _repository.UpdateJobOpeningUsers(jobOpeningId, assignedUserIds);

        // Only call GetReminder if new users are added (not removed)
        if (hasNewAssignedUsers) GetReminder(jobOpeningId);
    }
    ...
    public void GetReminder(int id)
    {
        ...
        var user = data.OrderBy(x => x).Last();
        ...
        switch (per.Role)
        {
            ...
            case "Technical":
                _emailSchedulerService.ScheduleForTechnical(per.Email, per.Fullname, per.Username, per.Password,
                    job.Title);
                break;
        }
